...
public class EmployeeTest3{
	...
	mts.dsc.orm.calendar.Calendar today;

	// DEFAULT CONSTRUCTOR
	public EmployeeTest3() {
	}
	
	// CONSTRUCTOR TO POPULATE EMPLOYEE WITH USER, POSITION, ETC
	public EmployeeTest3( mts.dsc.orm.user.User aUser ) {
	...
	public mts.dsc.activitycontainer.ActivityContainer[] getDateActivities(mts.dsc.orm.calendar.Calendar aDate) {
		...
		try {
			hours = 0;
			int count = 0;
			Object[] allActivities = aDate.getActivity().toArray();
			// COUNT MY ACTVITIES
			for ( int y = 0; y < allActivities.length; y ++ ) {
				mts.dsc.orm.activity.Activity anActivity = (mts.dsc.orm.activity.Activity) allActivities[y];
				if ( anActivity.getUseremp().getEmpId() == aUser.getEmpId() ) {
					count ++;
				}
			}
			loadTodaysActivities = new mts.dsc.activitycontainer.ActivityContainer[count];
			int actCount = 0;
			// LOAD MY ACTIVITIES
			for( int x = 0; x < allActivities.length; x++ ) {
				...
				if ( anActivity.getUseremp().getEmpId() == aUser.getEmpId() ) {
					...
					loadTodaysActivities[actCount] = curActivity;
					actCount++;
					hours += anActivity.getActualDuration();
					if (hours > getRequiredHours()) {
						overtime = hours - getRequiredHours();
		...
		try {
			...
			*/
			...
			try {
				...
				EmployeeTest3 anEmployee = new EmployeeTest3(testUser);
				EmployeeTest3 anEmployee1 = new EmployeeTest3(testUser1);
				EmployeeTest3 anEmployee2 = new EmployeeTest3(testUser2);
				EmployeeTest3 anEmployee3 = new EmployeeTest3(testUser3);
